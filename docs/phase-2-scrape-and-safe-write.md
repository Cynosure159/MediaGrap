...
Artwork download, Fanart.tv, provider caching/rate-limit persistence, write-plan cancellation/retry jobs, and field-level NFO merge choices remain subsequent Phase 2 increments. This first slice does not download image files; it only stores selected artwork URLs in the draft and NFO preview. Existing NFOs can be read as the initial draft, but they are not automatically persisted or merged with a provider result.
