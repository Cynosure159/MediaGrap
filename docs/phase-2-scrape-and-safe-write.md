...
Artwork download, Fanart.tv, provider caching/rate-limit persistence, write-plan cancellation/retry jobs, and existing-NFO merge/overwrite choices remain subsequent Phase 2 increments. This first slice never overwrites sidecars and does not download image files; it only stores selected artwork URLs in the draft and NFO preview.
