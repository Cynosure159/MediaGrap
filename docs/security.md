...
- Keep `/source` available to remote users of distributed/modified builds; it redirects only to the exact baked HTTPS GitHub Release source asset, never local state or a runtime-configured URL. This is a deliberate unauthenticated route. Health/readiness expose safe aggregate status too; scope proxy access to probes if needed without blocking the source offer.
