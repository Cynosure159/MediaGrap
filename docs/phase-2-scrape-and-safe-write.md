...
- Fanart.tv movie candidates accept a positive numeric TMDb ID from either a TMDb match or an imported NFO's explicitly typed `<uniqueid type="tmdb">`. NFO provenance is preserved; re-scraping is not required just to load artwork. Missing/invalid IDs and IDs belonging to other providers are rejected before requesting Fanart.tv. An IMDb ID is not interpreted as a TMDb ID.
