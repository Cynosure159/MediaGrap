...
## Directory-scoped scan snapshots

Scans enumerate each visited directory once and share its short-lived snapshot between direct video indexing and sidecar attribution. Direct files are processed before child directories; the snapshot is released before descent. Only pending child paths remain on the traversal stack, not a whole-library cache. Literal basename-prefix matching and single-main-video directory artwork/NFO ownership are unchanged, including sample exclusion and TV sidecars. Sidecar file types are still checked when attributed; read APIs obtain fresh directory listings rather than retaining scan snapshots.

Stage 1 introduced the directory snapshot optimization; see its [measurement methodology and local results](scan-directory-snapshots.md). Stage 2 adds [bounded repository batches and stat-based incremental scanning](incremental-scan-batches.md): unchanged videos still receive seen tracking, changed sidecars refresh the index, and eligible missing metadata is hydrated outside write transactions. Saved metadata priority and SQLite durability remain unchanged; full scans bypass fingerprints but never force-overwrite saved metadata. Stage 3 adds [two bounded ordered readers and persisted long-scan UI tracking](scan-readers-and-tracking.md), retaining one serialized scan writer and atomic saved-metadata priority.
...
Observed persisted scan terminal transitions refresh the catalog and recreate the selected inspector so removed files/episodes cannot remain as stale detail content. Tracking survives navigation/reload and does not expire after 60 seconds; failed/cancelled/interrupted jobs refresh partial committed results without reconciling missing files. External filesystem deletions are reflected after a successful source scan, not by continuous filesystem watching.
