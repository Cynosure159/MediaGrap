...
func (a *Asset) UnmarshalJSON(data []byte) error {
	...
	a.ID, a.URL, a.Lang, a.Added = raw.ID, raw.URL, raw.Lang, raw.Added
...
func (c *Client) configured() (string, string, string, *http.Client, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	if c.apiKey == "" {
...
func (c *Client) Movie(ctx context.Context, tmdbID string) ([]Asset, error) {
	apiKey, language, endpoint, client, err := c.configured()
	if err != nil {
		return nil, err
	}
	...
	assets := make([]Asset, 0)
	for _, group := range artworkGroups {
	...
	sort.SliceStable(assets, func(i, j int) bool {
		if artworkKindRank(assets[i].Kind) != artworkKindRank(assets[j].Kind) {
			return artworkKindRank(assets[i].Kind) < artworkKindRank(assets[j].Kind)
		}
...
func artworkKindRank(kind string) int {
	for index, group := range artworkGroups {
		if group[1] == kind {
			return index
		}
	}
	return len(artworkGroups)
}
