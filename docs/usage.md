...
The movie File Audit tab may show a read-only, real-time supplemental view of direct sample videos and direct videos in case-insensitive exact `Extra`/`Extras` directories. It associates these rows only when the selected movie is the sole regular non-sample main video in its immediate parent; shared roots, multiple main videos, missing mains, symlink entries, unreadable directories and safety-limit results are shown as not associated, unreadable, incomplete or truncated rather than guessed. The view is bounded, does not recurse into nested directories, does not probe video contents, and these rows are never included in rename or NFO plans.
