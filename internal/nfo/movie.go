...
// Movie represents the Kodi-compatible movie NFO schema.
type Movie struct {
	Title         string
...
func ParseMovie(input []byte) (Movie, error) {
	...
	movie := Movie{
		...
		Votes:         decoded.Votes,
		ContentRating: strings.TrimSpace(decoded.ContentRating),
		Directors:     cleaned(decoded.Directors),
		Writers:       cleaned(decoded.Writers),
...
func WriteMovie(movie Movie) ([]byte, error) {
	encoded := movieXML{
		...
		Genres:        cleaned(movie.Genres),
		Rating:        movie.Rating,
		Votes:         movie.Votes,
		ContentRating: strings.TrimSpace(movie.ContentRating),
		Directors:     cleaned(movie.Directors),
		Writers:       cleaned(movie.Writers),
		Studios:       cleaned(movie.Studios),
	}
