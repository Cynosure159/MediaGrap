...
func ParseMovie(input []byte) (Movie, error) {
	...
	movie := Movie{Title: strings.TrimSpace(decoded.Title), OriginalTitle: strings.TrimSpace(decoded.OriginalTitle), Year: decoded.Year, Plot: strings.TrimSpace(decoded.Plot), Runtime: decoded.Runtime, Genres: decoded.Genres}
...
func WriteMovie(movie Movie) ([]byte, error) {
	encoded := movieXML{Title: strings.TrimSpace(movie.Title), OriginalTitle: strings.TrimSpace(movie.OriginalTitle), Year: movie.Year, Plot: strings.TrimSpace(movie.Plot), Runtime: movie.Runtime, Genres: cleaned(movie.Genres)}
	if movie.TMDbID != "" {
