...
| Phase 5A | 已完成首个可用切片 | 持久化作业事件、运行中取消、失败/取消/中断任务重试、支持断线重连的 SSE、统一写入审计查询、系统状态聚合，以及响应式作业与运维中心。定时扫描、SOCKS5/NO_PROXY 设置、真实 Provider 连接测试、备份恢复和父子批处理仍待后续切片。 |
...
当前首个切片已完成作业取消/重试、持久化事件与 SSE 重连、审计查询、脱敏系统状态和桌面/移动运维中心；详细边界见 [Phase 5A 作业与运维可见性](phase-5a-operations.md)。来源定时策略、SOCKS5/NO_PROXY、真实 Provider 测试、服务端界面偏好、备份恢复与父子批处理继续保留在本阶段后续切片。
