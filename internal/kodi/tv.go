...
// Unknown extension fields are intentionally not copied during a replacement.
type TVShow struct {
	Title         string
...
func ParseTVShow(input []byte) (TVShow, error) {
	...
	show := TVShow{Title: strings.TrimSpace(decoded.Title), OriginalTitle: strings.TrimSpace(decoded.OriginalTitle), Year: decoded.Year, Plot: strings.TrimSpace(decoded.Plot), Genres: cleaned(decoded.Genres), Rating: decoded.Rating, Votes: decoded.Votes, Status: strings.TrimSpace(decoded.Status), Network: strings.TrimSpace(decoded.Network)}
...
func WriteTVShow(show TVShow) ([]byte, error) {
	encoded := tvShowXML{Title: strings.TrimSpace(show.Title), OriginalTitle: strings.TrimSpace(show.OriginalTitle), Year: show.Year, Plot: strings.TrimSpace(show.Plot), Genres: cleaned(show.Genres), Rating: show.Rating, Votes: show.Votes, Status: strings.TrimSpace(show.Status), Network: strings.TrimSpace(show.Network)}
	if show.TMDbID != "" {
...
func ParseEpisode(input []byte) (Episode, error) {
	...
	episode := Episode{Title: strings.TrimSpace(decoded.Title), Plot: strings.TrimSpace(decoded.Plot), Season: decoded.Season, Episode: decoded.Episode, AirDate: strings.TrimSpace(decoded.AirDate), Runtime: decoded.Runtime}
	for _, id := range decoded.UniqueID {
		if strings.EqualFold(id.Type, "tmdb") {
...
func WriteEpisode(episode Episode) ([]byte, error) {
	encoded := episodeXML{Title: strings.TrimSpace(episode.Title), Plot: strings.TrimSpace(episode.Plot), Season: episode.Season, Episode: episode.Episode, AirDate: strings.TrimSpace(episode.AirDate), Runtime: episode.Runtime}
	if episode.TMDbID != "" {
