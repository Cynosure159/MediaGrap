...
export interface TVShow { id: number; sourceId: number; relativePath: string; titleHint: string; yearHint: number | null; episodeCount: number; seasonCount: number }
export interface TVEpisode extends MediaItem { seasonNumber: number; episodeStart: number; episodeEnd: number }
export interface TVShowDetail { show: TVShow; episodes: TVEpisode[]; writable: boolean }
...
export const tvShows = (q = '') => request<{ items: TVShow[] }>(`/api/v1/tv/shows?q=${encodeURIComponent(q)}`)
export const tvShowDetail = (id: number) => request<TVShowDetail>(`/api/v1/tv/shows/${id}`)
