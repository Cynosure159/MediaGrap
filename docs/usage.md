...
Scans are read-only for media/NFO/artwork, write only the SQLite index and eligible local metadata, and do not call providers automatically. Accepted video extensions are `mkv`, `mp4`, `m4v`, `avi`, `mov`, `webm`; symlinks are skipped. TV examples: `Show.S02E03.mkv`, `Show - 1x02 - Pilot.mp4`, `Show.S01E01E02.mkv`. Conventional `Show/Season 01/` directories supply show hints; Season 0 is supported. Other indexed videos appear as movies. One multi-episode file currently uses the first recognized episode for display/NFO; this is not full multi-episode NFO or official missing-episode reporting.
...
- **Full** bypasses fingerprints and revalidates index classification; it does not force local NFO edits over saved metadata.
